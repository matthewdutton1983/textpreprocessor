...
@pipeline_method
def replace_words(input_text: str, replacement_dict: Dict[str, str], case_sensitive: bool = False) -> str:
    if case_sensitive:
...
@pipeline_method
def make_lowercase(input_text: str) -> str:
    return input_text.lower()


@pipeline_method
def make_uppercase(input_text: str) -> str:
    return input_text.upper()


@pipeline_method
def remove_numbers(input_text: str) -> str:
    return re.sub('\d+', '', input_text)


@pipeline_method
def remove_list_markers(input_text: str) -> str:
    """Remove bullets or numbering in itemized input"""
    return re.sub('(^|\s)[0-9a-zA-Z][.)]\s+|(^|\s)[ivxIVX]+[.)]\s+', ' ', input_text)


@pipeline_method
def remove_urls(input_text: str, use_mask: Optional[bool] = True, custom_mask: Optional[str] = None) -> str:
...
@pipeline_method
def remove_duplicate_punctuation(input_text: str) -> str:
    return re.sub(r'([\!\?\.\,\:\;]){2,}', r'\1', input_text)


@pipeline_method
def remove_special_characters(input_text: str, remove_unicode: bool = False, custom_characters: Optional[List[str]] = None) -> str:
    if remove_unicode:
...
@pipeline_method
def remove_email_addresses(input_text: str, use_mask: Optional[bool] = True, custom_mask: Optional[str] = None) -> str:
    regex_pattern = re.compile(
...
@pipeline_method
def check_spelling(input_text_or_list: Union[str, List[str]], case_sensitive: bool = True, language: str = 'en') -> str:
...
@pipeline_method
def tokenize_sentences(input_text: str) -> List[str]:
...
@pipeline_method
def remove_names(input_text: str, use_mask: Optional[bool] = True, custom_mask: Optional[str] = None) -> str:
    if custom_mask:
        use_mask = True
        mask = custom_mask
    else:
...
@pipeline_method
def remove_whitespace(input_text: str, mode: str = 'strip', keep_duplicates: bool = False) -> str:
...
@pipeline_method
def remove_phone_numbers(input_text: str, use_mask: Optional[bool] = True, custom_mask: Optional[str] = None) -> str:
    regex_pattern = re.compile(
...
@pipeline_method
def encode_text(input_text: str, encoding: str = 'utf-8', errors: str = 'strict') -> bytes:
...
@pipeline_method
def remove_social_security_numbers(input_text: str, use_mask: Optional[bool] = True, custom_mask: Optional[str] = None) -> str:
...
@pipeline_method
def remove_credit_card_numbers(input_text: str, use_mask: Optional[bool] = True, custom_mask: Optional[str] = None) -> str:
...
@pipeline_method
def stem_words(input_text_or_list: Union[str, List[str]], stemmer: Optional[str] = None) -> List[str]:
...
@pipeline_method
def lemmatize_words(input_text_or_list: Union[str, List[str]]) -> List[str]:
...
@pipeline_method
def handle_line_feeds(input_text: str, mode: str = 'remove') -> str:
...
@pipeline_method
def find_abbreviations(input_text: str) -> Dict[str, str]:
